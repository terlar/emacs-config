...
package. Currently GNU Emacs 28.0.50.
...
- minibuffer completion :: [[https://github.com/raxod502/selectrum][selectrum]], [[https://github.com/abo-abo/swiper][ivy]]
...
(use-package selectrum
  :ensure t
  :preface
  (autoload 'selectrum-exhibit "selectrum")
  (autoload 'selectrum-insert-current-candidate "selectrum")
...
(use-package ivy
  ...
  :init
  (setq ivy-hooks-alist '((t . hl-line-mode)))
  (setq ivy-wrap t)
...
(use-package consult
  ...
  (:prefix search-prefix "g" 'consult-ripgrep)
  :init
...
(use-package ctrlf
  ...
  :config
  ;; Fix annoying issue with jumping to the wronge place when having split windows and
  ;; searching.
  (advice-add 'ctrlf--finalize :override #'ignore))
#+end_src
...
*** format-all
Auto-format source code with wide support for languages.
#+begin_src emacs-lisp :tangle yes
...
(use-package org-eldoc
  :after eldoc
  :preface
  (defun init-org-eldoc ()
    "Initialize `org-eldoc'."
    (org-eldoc-load)
    (eldoc-mode 1))
...
(use-package org
  ...
  :hook
  (org-mode-hook . init-org-prettify-syntax)
...
(use-package completing-read-xref
  :ensure t
  :commands
  (completing-read-xref-show-defs completing-read-xref-show-xrefs)
  :init
