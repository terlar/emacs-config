...
** Text flow
   ...
   (defun +truncate-lines-on ()
     "Truncate long lines for the current buffer."
     (setq truncate-lines t))
   (add-hook 'visual-line-mode-hook #'+truncate-lines-on)
...
*** Other file
    ...
    #+BEGIN_SRC emacs-lisp
    (defun +emacs-lisp-mode-setup-find-file ()
      "Setup `ff-find-other-file' for `emacs-lisp-mode'."
      (setq ff-other-file-alist '(("-test\\.el$" (".el"))
                                  ("\\.el$" ("-test.el")))))
    (add-hook 'emacs-lisp-mode-hook #'+emacs-lisp-mode-setup-find-file)
    #+END_SRC
...
*** REPL
    ...
    #+BEGIN_SRC emacs-lisp
    (defvar +haskell-interactive-mode-global-history nil)
    (with-eval-after-load 'savehist
      (cl-pushnew '+haskell-interactive-mode-global-history savehist-additional-variables :test #'equal))

    (defun +haskell-interactive-mode-load-history ()
      "Restore `haskell-interactive-mode' history."
      (add-hook 'kill-buffer-hook #'+haskell-interactive-mode-save-history nil t)
      (when +haskell-interactive-mode-global-history
        (setq haskell-interactive-mode-history +haskell-interactive-mode-global-history)))

    (defun +haskell-interactive-mode-save-history ()
      "Save `haskell-interactive-mode' history."
      (setq +haskell-interactive-mode-global-history haskell-interactive-mode-history))

    (add-hook 'haskell-interactive-mode-hook #'+haskell-interactive-mode-load-history)
    #+END_SRC
...
** js
   ...
   (use-package js2-mode
     ...
     "nodejs"
     :hook
     (js2-mode . +js-mode-setup-find-file)
     ...
     (:keymaps
      ...
      "r" 'js-repl)
     :preface
     (defun +js-mode-setup-find-file ()
       "Setup `ff-find-other-file' for `js2-mode'."
...
** lisp
   ...
   (use-package lisp-mode
     :hook
     (lisp-mode . +lisp-mode-setup-find-file)
     :preface
     (defun +lisp-mode-setup-find-file ()
       "Setup `ff-find-other-file' for `lisp-mode'."
...
** term
    ...
    (use-package term
      :hook
      (ansi-term . +ansi-term-setup)
      :preface
      (defun +ansi-term-setup ()
        "Setup for `ansi-term'."
...
** typescript
   ...
   (use-package typescript-mode
     :hook
     (typescript-mode . +typescript-mode-setup-find-file)
     (typescript-mode . +typescript-mode-prettify-symbols)
     :preface
     (defun +typescript-mode-setup-find-file ()
       "Setup `ff-find-other-file' for `typescript-mode'."
       (setq ff-other-file-alist
             ...
             (append '(".") (ff-test-project-dirs '("src"
                                                    ...
                                                    "spec" "spec/*"
                                                    "specs" "specs/*")))))
     (defun +typescript-mode-prettify-symbols ()
       (dolist (symbol '(("=="        . ?≈)
                         ("=>"        . (?- (Br . Bc) ?- (Br . Bc) ?>))
                         ("function"  . ?ƒ)
                         ("let"       . ?ν)
