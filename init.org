...
*** Padding
    Add margins inside windows to make text feel less crowded. Put fringes on the outside
    for the same reason. Padding around the frame is configured via the
    =internal-border-width= in the [[Frame][Frame]] section.
    #+BEGIN_SRC emacs-lisp
