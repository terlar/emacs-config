...
(use-package local-theme
  :demand t
  :hook
  (after-load-theme . local-theme-ensure)
  (server-after-make-frame . local-theme-ensure)
  :custom
  ...
  (defun load-theme-run-after-load-theme-hooks (&rest _)
...
(use-package grugru
  ...
  (:prefix
   init-next-prefix
   "r" '(grugru-repeat :wk "Rotate text"))
  (:prefix
   init-prev-prefix
   "r" '(grugru-repeat :wk "Rotate text"))
  :preface
...
- Restore no files to begin with to avoid slow starts with huge sessions.
#+BEGIN_SRC emacs-lisp
(use-package desktop
  :hook
  (server-after-make-frame . desktop-save-mode)
  :custom
