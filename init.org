...
Inhibit startup screen and messages. If you are new to GNU Emacs it is recommended to not
...
;; FIXME: Synthetic theme for :custom variables. Required as `use-package' is only
  ...
  (defun server-ensure-running (frame)
    ...
    (with-selected-frame frame
      (unless (server-running-p)
        (server-start))))
  :hook
  (after-make-frame-functions . server-ensure-running))
#+end_src
...
(use-package selectrum
  ...
  (selectrum-mode-hook . selectrum-define-keys)
  :custom
...
(use-package consult
  ...
  :custom
  (consult--fd-cmd '("fd" "--color=never" "--full-path"))
  :general
