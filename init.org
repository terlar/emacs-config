...
(defun time-from-work-log-file ()
  "Extract time from work-log file."
...
(defun work-log-file (&optional time)
  ...
  (interactive)
  (let ((target-date (format-time-string "%Y-%m-%d" time)))
    (find-file (expand-file-name (concat (file-truename work-log-directory)
                                         "/"
                                         (format work-log-file-name-pattern target-date))))))

(defun work-log-file-previous ()
  "Open previous work log file. If not work log file, open today's work log file."
  (interactive)
  (let ((time (time-from-work-log-file))
        (day-in-seconds (* 24 3600)))
    (if time
        (work-log-file (time-subtract time day-in-seconds))
      (work-log-file))))

(defun work-log-file-next ()
  "Open next work log file. If not work log file, open today's work log file."
  (interactive)
  (let ((time (time-from-work-log-file))
        (day-in-seconds (* 24 3600)))
    (if time
        (work-log-file (time-add time day-in-seconds))
      (work-log-file))))
...
**** Task lists
Add extra face for =TODO= headline.
#+BEGIN_SRC emacs-lisp
(defface org-headline-todo
  '((t (:bold nil)))
  "Face used to indicate that a headline is a TODO.
This face is only used if `org-fontify-todo-headline' is set.  If applies
to the part of the headline after the TODO keyword."
  :group 'org-faces)

(defun +org-set-todo-font-lock ()
  "Fontify TODO headlines."
  (font-lock-add-keywords 'org-mode
                  `((,(format org-heading-keyword-regexp-format
                              (concat
                               "\\(?:"
                               (mapconcat 'regexp-quote (or org-not-done-keywords) "\\|")
                               "\\)"))
                     (2 'org-headline-todo t)))))
(advice-add 'org-set-font-lock-defaults :after #'+org-set-todo-font-lock)
#+END_SRC
...
** TODO Work Log next/previous with =]w= and =[w=.
