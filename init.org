...
** Functions
   Tangle and byte compile org file.
   #+BEGIN_SRC emacs-lisp
   (defun org-babel-tangle-elisp ()
     "Tangle current org file into emacs-lisp."
     (interactive)
     (let ((file-source buffer-file-name)
           (file-target (concat (file-name-sans-extension buffer-file-name) ".el")))
       (eval-when-compile
         (require 'org))
       (org-babel-tangle-file file-source file-target "emacs-lisp")))

   (defun org-babel-tangle-elisp-and-byte-compile ()
     "Tangle current org file into emacs-lisp and byte compile."
     (interactive)
     (org-babel-tangle-elisp)
     (let ((file-target (concat (file-name-sans-extension buffer-file-name) ".el")))
       (byte-compile-file file-target)))

   (defun async-org-babel-tangle-elisp ()
     "Tangle current org file into emacs-lisp asynchronously."
     (interactive)
     (let ((init-tangle-start-time (current-time))
           (file-source buffer-file-name)
           (file-target (concat (file-name-sans-extension buffer-file-name) ".el")))
       (eval-when-compile
         (require 'async))
       (async-start
