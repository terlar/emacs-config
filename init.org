...
**** Next
     ...
     (general-define-key
      ...
      "w" `(,(defrepeater #'eyebrowse-next-window-config) :wk "Workspace"))
     #+END_SRC
...
**** Previous
     ...
     (general-define-key
      ...
      "w" `(,(defrepeater #'eyebrowse-prev-window-config) :wk "Workspace"))
     #+END_SRC
...
** Auto save
   ...
   Ensure =save-some-buffers= doesn't destroy the message in Echo Area.
   #+BEGIN_SRC emacs-lisp
   (advice-add #'save-some-buffers :around #'quiet-function-advice)
   #+END_SRC
...
** Project
   ...
   (use-package projectile
     ...
     (:keymaps
      ...
      "C-x p" '(:keymap projectile-command-map :package projectile :wk "project"))
     :preface
...
*** editorconfig
    ...
    (use-package editorconfig
      ...
      :preface
      (defun +editorconfig-set-trailing-ws-use-ws-butler-advice (orig trim-trailing-ws)
        "Use ws-butler mode instead of delete-trailing-whitespace.
        Never calls ORIG function with TRIM-TRAILING-WS."
        ...
        (when (or (equal trim-trailing-ws "false")
          ...
          (ws-butler-mode 0)))
      :config
      (advice-add 'editorconfig-set-trailing-ws :around #'+editorconfig-set-trailing-ws-use-ws-butler-advice)
      (editorconfig-mode 1))
    #+END_SRC
...
** org
   Ensure correct Org version is installed with =straight.el=. See [[https://github.com/raxod502/straight.el#installing-org-with-straightel][this link]] for further
   information.
   #+BEGIN_SRC emacs-lisp
   (require 'subr-x)
   (use-package git)
...
** ocaml
   ...
   (use-package caml
     :straight
     (:host github :repo "ocaml/ocaml" :branch "trunk" :files ("emacs/*.el")))
...
**** Bullet lists
     ...
     (defun +rst-mode-prettify-checkbox-symbols ()
       "Prettify checkbox symbols."
       (dolist (symbol '(("[ ]" . ?□)
                         ("[x]" . ?▣)
                         ("[X]" . ?▣)))
         (cl-pushnew symbol prettify-symbols-alist :test #'equal)))
     (add-hook 'rst-mode-hook #'+rst-mode-prettify-checkbox-symbols)
     #+END_SRC
...
**** ensime
     ...
     (use-package ensime
       :straight
       (:host github :repo "ensime/ensime-emacs" :branch "2.0")
       :hook
...
** TODO Better key for ivy-yasnippet?
** TODO Better key for counsel-tramp
