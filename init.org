...
(use-package cape
  ...
  (defun init-cape-capf ()
    ...
    (cl-pushnew #'cape-symbol completion-at-point-functions :test #'equal)
...
(use-package apheleia
  ...
  :init
  (eval-after-load 'apheleia-formatters
    '(progn
       ...
       (dolist (mode-formatter '((nix-mode . nix)
                                 (ruby-mode . rufo)))
         (cl-pushnew mode-formatter apheleia-mode-alist :test #'equal)))))
#+end_src
...
(use-package eglot
  ...
  :config
  (cl-pushnew '(elixir-mode . ("elixir-ls")) eglot-server-programs :test #'equal))
#+end_src
